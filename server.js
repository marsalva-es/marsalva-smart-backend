...
// ✅ FIX: antes interpretaba "afternoon" como "morning"
function normalizeBlock(b) {
  ...
  // Soporte español por si llega "tarde/mañana"
  if (s.includes("tard")) return "afternoon";
  if (s.includes("mañ") || s.includes("mana")) return "morning";
...
// OJO: aquí tu lógica real debe estar igual que en la V10.
// Yo NO la toco. Solo añado un log y normalizo el bloque.

app.post("/availability-smart", async (req, res) => {
    // ✅ Log para ver qué llega desde la web y qué se usa
    const rawBlock = req.body?.block;
    const block = normalizeBlock(rawBlock);
    console.log("🔎 availability-smart block recibido:", rawBlock, "-> normalizado:", block);
